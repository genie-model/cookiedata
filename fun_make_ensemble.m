...
%   (3-n) any number of parameter values [real OR string]
...
%
%   1 go_0 0.0E-3 10.0E-3 100.0E-3
%   2 ea_0 1.0E6 0.8E6 0.6E6 0.4E6 0.2E6 0.0E6
%
%   would create a 1 x 6 x 3 ensemble,
%   where the x- (1st) axis values are defined by the vector [0.0E-3 10.0E-3 100.0E-3]
%   and the y- (2nd) axis values by the vector [1.0E6 0.8E6 0.6E6 0.4E6 0.2E6 0.0E6]
%
%   EXAMPLE 2
%
%   The parameter configuration file contents:
%
%   1 go_0 0.0E-3 10.0E-3 100.0E-3
%   1 go_9 1.0E-9 2.0E-9 3.0E-9
%   2 ea_0 1.0E6 0.8E6 0.6E6 0.4E6 0.2E6 0.0E6
%
%   would create a 1 x 6 x 3 ensemble as per example #1, 
...
%   1 go_0 0.0E-3 10.0E-3 100.0E-3
%
%   creates a 1 x 1 x 3 ensemble in which only one parameter varies.
%
%   ***********************************************************************
%   *** HISTORY ***********************************************************
%   ***********************************************************************
...
for l=[1:l_lines]
    ...
    %s(l).vstring = string(loc_c{3:n_var});
    for m=[3:n_var]
        s(l).vstring(m-2) = string(loc_c{m});
    end
    % loc_v = [];
    % for m=[4:n_var]
...
for d=1:par_dmax
    loc_v = find([s(:).id] == d);
    loc_p = loc_v(1);
    par_vmax(d) = length(s(loc_p).vstring);
end
%
...
for o=1:par_vmax(3)
    for n=1:par_vmax(2)
        for m=1:par_vmax(1)
            ...
            for p=1:par_pmax
                % add comment line
                loc_str = ['# parameter = ' num2str(p) ', value = ' num2str(loc_vn(s(p).id))];
                fprintf(fid, '%s\n', loc_str);
                % add parameter line
                % check for first character being a digit
                loc_char = char(s(p).vstring(loc_vn(s(p).id)));
                if (isstrprop(loc_char(1), 'digit'))
                    loc_str = [s(p).paramname '=' loc_char];
                else
                    loc_str = [s(p).paramname '=' '"' loc_char '"'];
                end
                fprintf(fid, '%s\n', loc_str);
...
for p=1:par_pmax
    ...
    fprintf(fid0, '%s\n', loc_str);    
    loc_str = ['     parameter values                 : ' char(join(s(p).vstring))];
    fprintf(fid0, '%s\n', loc_str);    
end
loc_str = [' ***********************************************************'];
fprintf(fid0, '%s\n', loc_str);
